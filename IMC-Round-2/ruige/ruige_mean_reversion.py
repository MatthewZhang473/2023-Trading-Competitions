...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        product_orders = {key:None for key in products}

        for product in products:
            order_depth = state.order_depths[product]
            orders = state.order_depths[product]
            buy_prices = list(orders.buy_orders.keys())
            sell_prices = list(orders.sell_orders.keys())
            mean_price = (sum(buy_prices) + sum(sell_prices)) / (len(buy_prices) + len(sell_prices))

            # Determine whether to buy or sell based on the current position
            if product not in state.position.keys():
                print(f'{product} not in state')
                current_position = 0
            else:
                current_position = state.position[product]

            print(f"Current position in {product}: {current_position}")
            if current_position >= 0:
                # If we're currently long on pearls, sell if the price is above the mean
                if sell_prices and sell_prices[0] > mean_price:
                    sell_quantity = min(limits[product] + current_position, order_depth.sell_orders[sell_prices[0]])
                    print(f"Selling {sell_quantity} {product} at {sell_prices[0]}")
                    product_orders[product] = Order(product, sell_prices[0], -sell_quantity)
            else:
                # If we're currently short on pearls, buy if the price is below the mean
                if buy_prices and buy_prices[-1] < mean_price:
                    buy_quantity = min(limits[product] - current_position, order_depth.buy_orders[buy_prices[-1]])
                    print(f"Buying {buy_quantity} {product} at {buy_prices[-1]}")
                    product_orders[product] = Order(product, buy_prices[-1], buy_quantity)
            # If we didn't send any orders or the product is not being traded, return an empty result
        ...
        arbitrage_COCONUTS_orders, arbitrage_PINAS_orders =\
            self.arbitrage_calc(product_1=COCONUTS, product_2=PINA_COLADAS,
                                ...
                                '''
        result['COCONUTS'] = [product_orders['COCONUTS']]
        result['PINA_COLADAS'] = [product_orders['PINA_COLADAS']]
        self.logger.log(
            f"Making COCONUTS orders by mean reversion:\n{result['COCONUTS']}", "orders")
        self.logger.log(
            f"Making PINAS orders by mean reversion:\n{result['PINA_COLADAS']}", "orders")
    ...
    def arbitrage_calc(self, product_1: Product, product_2: Product,
        ...
        if best_bid_price_product_1 > best_ask_price_product_2 * product_1_over_product_2_value_ratio:
            ...
            if best_bid_price_product_1 * lot_size_product_1 - best_ask_price_product_2 * lot_size_product_2 > big_diff:
                ...
                # how many lots of product 2 can I buy from the order book, without breaching position limit
                max_number_buy_lots = min(best_ask_volume_product_2,\
                                        position_limit_product_2*reserve_proportion - position_product_2)//lot_size_product_2

                # how many lots of product 1 can I sell from the order book, without breaching position limit
                max_number_sell_lots = min(best_bid_volume_product_1,\
                                        position_limit_product_1*reserve_proportion + position_product_1)//lot_size_product_1
