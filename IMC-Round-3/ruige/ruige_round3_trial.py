...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        diving_gear_orders = self.indicator_trade(product, indicator_mid_price, best_bid_price, 
                                                        best_bid_volume, best_ask_price, 
    ...
    def indicator_trade(self, product, indicator_mid_price, 
                        best_bid_price, best_bid_volume, best_ask_price, 
                        best_ask_volume, product_position, product_position_limit):
        # push the current price into the indicator window
        self.dolphin_window.push(indicator_mid_price)

        # create a difference series, containing the differences between ith and i+30th elements
        differences_series = pd.Series(self.dolphin_window.contents).diff(periods = 30)
        current_difference = differences_series.iloc[-1]
        past_differences_series = differences_series.iloc[30:60]

        sigma = past_differences_series.std()
        past_mean = past_differences_series.mean()
        ...
        # condition to buy
        if current_difference - 1.5*sigma > past_mean and current_difference > absolute_thres:
            buy_volume = min(best_ask_volume, product_position_limit - product_position)
            if buy_volume != 0:
                product_orders.append(Order(product, best_ask_price, buy_volume))
        
        # condition to sell
        elif current_difference + 1.5 * sigma < past_mean and current_difference < -absolute_thres:
            sell_volume = min(best_bid_volume, product_position_limit + product_position)
            if sell_volume != 0:
                product_orders.append(Order(product, best_bid_price, -sell_volume))
